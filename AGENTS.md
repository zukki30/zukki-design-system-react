...
Reach for React types and values through named imports from `react` (`import type { ReactNode } from 'react'`), never through the `React` namespace (`React.ReactNode`, `React.useState`, `<React.Fragment>`). `@types/react` declares `React` as a UMD global, so a namespace reference type-checks even with no import in the file, which quietly turns into a dependency on that global. ESLint's `no-restricted-syntax` enforces this for type positions, value positions, and JSX.
...
Give title parts (`Card.Title`, `Dialog.Title`) a `level?: HeadingLevel` prop that switches between `h2`–`h6`, instead of telling consumers to pair `role="heading"` with `aria-level` — passing only one of the two produces invalid ARIA the library cannot guard against. `HeadingLevel` (`2 | 3 | 4 | 5 | 6`) is shared across components, so it lives in `src/types`; `h1` is the page's own heading and stays out of it. Convert a level to a tag name with `headingTag()` from `@/utils/headingTag`.

Whether the part renders a heading by default depends on whether the library can know the right level. `Card.Title` falls back to `div` with no heading semantics, because a card's depth in the document outline is unknown to the library. `Dialog.Title` falls back to `h2`, because a modal's background is inert and the title is always the top of that self-contained context. When a part can render either a `div` or a heading, widen `ref` to `HTMLElement` (`Omit<ComponentPropsWithRef<'div'>, 'ref'> & { ref?: Ref<HTMLElement> }`); parts that always render a heading keep `ComponentPropsWithRef<'h2'>`, since `h2`–`h6` are all `HTMLHeadingElement`. Build variable-tag output with `createElement()` rather than JSX: JSX would require assigning the tag to a capitalised variable, which `react-hooks/static-components` reads as creating a component during render even though the value is just a tag-name string.
