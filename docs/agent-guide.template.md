...
- **フォーム部品の `size` は `sm` / `md` の 2 段階です。** `<FormField size="sm">` を使うと、配下の入力要素がまとめて小さくなります。入力要素側で `size` を指定した場合はそちらが優先されます。`sm` の密度は余白と付属パーツで作っているため、**入力欄の文字は 16px のまま**（それより小さいと iOS Safari がフォーカス時にページを自動ズームするため）、`FormField` のラベル（14px）と補助テキスト（12px）のサイズも変わりません
