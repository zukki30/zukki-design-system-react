...
- アイコンの描画サイズは現状の出し分けを引き継ぐ（`md` 16px / `sm` 14px）
  - 設計フェーズでは `md` 24px / `sm` 20px へ上げることで一度合意したが、**実装後のレビューでこの要件どおり 16 / 14 に戻した**。経緯は [`design.md` の「要件からの変更点（実装後に差し戻し）」](./design.md#要件からの変更点実装後に差し戻し) と [`results.md`](./results.md) を参照
  - 当たり判定 24px はボタン自身の `width` が持つため、アイコンを大きくしなくても R1 は満たせる
