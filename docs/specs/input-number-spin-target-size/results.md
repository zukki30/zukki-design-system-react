...
## アイコンサイズは 16 / 14 のまま据え置いた（設計から差し戻し）

設計フェーズでは `md` 24px / `sm` 20px へ上げることで合意していたが、**実装後のレビューで 16 / 14 に戻した**。
...
一方で、実際に並べてみると 24px の `−` / `+` は入力欄の数値に対して主張が強かった。**スピンボタンはポインタ専用の補助**（`tabIndex={-1}`）であり、入力欄より目立つ必要はない。当たり判定 24px はボタン自身の `width` が持っているので、**アイコンを大きくしないと基準を満たせないわけではない**。

`sm` を 20px のまま `md` だけ 16px にすると `sm` のアイコンが `md` より大きくなるため、両方を下げて元の関係に戻した。
...
結果として `#115` 時点と同じ値になっている。変わったのはグリフ（`menuUp` / `menuDown` → `baselineMinus` / `plus`）とボタンの当たり判定だけ。
